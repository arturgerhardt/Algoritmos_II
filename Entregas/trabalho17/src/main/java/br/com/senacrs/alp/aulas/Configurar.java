...
import br.com.senacrs.alp.aulas.InterpretarStrings.TIPO_STRING;

public class Configurar implements ArquivoConfiguracao {
	...
	Configurar(String arquivoConfiguracao) {
		if (arquivoConfiguracao == null) {
			throw new IllegalArgumentException(
					"O nome do arquivo não pode ser nulo.");
		}

		File f = new File(arquivoConfiguracao);
		...
		try {
			chaves = leitorArqivo.lerArquivo(arquivoConfiguracao);
		} catch (Exception ex) {
			throw new IllegalArgumentException(
		...
		InterpretarStrings validador = new Interpretador();
